...
00000000004008b6 <popcountb>:
  4008b6:       83 e6 07                and    $0x7,%esi
  4008b9:       31 c0                   xor    %eax,%eax
  4008bb:       f3 48 0f b8 17          popcnt (%rdi),%rdx
  4008c0:       48 83 c7 10             add    $0x10,%rdi
  4008c4:       83 c6 07                add    $0x7,%esi
  4008c7:       f3 48 0f b8 4f f8       popcnt -0x8(%rdi),%rcx
  4008cd:       48 c1 e0 08             shl    $0x8,%rax
  4008d1:       83 e6 07                and    $0x7,%esi
  4008d4:       48 01 ca                add    %rcx,%rdx
  4008d7:       48 01 d0                add    %rdx,%rax
  4008da:       40 80 fe 07             cmp    $0x7,%sil
  4008de:       75 db                   jne    4008bb <popcountb+0x5>
  4008e0:       c3                      retq   

  400799:       31 d2                   xor    %edx,%edx
  40079b:       31 c0                   xor    %eax,%eax
  40079d:       f3 48 0f b8 0f          popcnt (%rdi),%rcx
  4007a2:       48 83 c7 10             add    $0x10,%rdi
  4007a6:       48 c1 e0 08             shl    $0x8,%rax
  4007aa:       f3 4c 0f b8 47 f8       popcnt -0x8(%rdi),%r8
  4007b0:       ff c2                   inc    %edx
  4007b2:       4c 01 c1                add    %r8,%rcx
  4007b5:       48 01 c8                add    %rcx,%rax
  4007b8:       39 f2                   cmp    %esi,%edx
  4007ba:       7c e1                   jl     40079d <popcount128+0x4>
  4007bc:       c3                      retq   
  4007bd:       0f 1f 00                nopl   (%rax)

* TTA still exists! 

This is quite far from horrible at first glance.

http://tce.cs.tut.fi/

Aside from that I need to pick up VHDL. And C++. But C++ is a halfway decent map
to this....
...
* And specc is not dead, but resting

http://www.cecs.uci.edu/~specc/reference/

I simply do not know enough about it or systemC to get anywhere
at the moment. When last I tried systemC it was horrible.

All I really wanted was to be able to pack clearly defined bit
structures into other things, and unpack them. Cleanly. And be able to
use the new bit extraction stuff in the latest intel cpus.

Sigh. Another preprocessor awaits.

1) I don't need (or want) cycle accurate simulation - I want the
   "simulation" to run as fast as possible in a real cpu.
1a) Don't got clocks. Don't need no stinking clocks.

2) I DO want things to compile down to hardware.

Another issue I have with C++ is you can't define new operators entirely, 
you can only overload them. That's fixible, but....

Anyway, despite me saying that, systemc does look a great deal 
more friendly to my eye than pure vhdl or verilog.

http://www.asic-world.com/systemc/data_types2.html#Example_sc_bit

* Started ebusl

While I'm coping with scary things, I thought I'd take a quick look at
musl, and see just how big the errno portion of the problem was. So I
forked it to "ebusl" on my github and did a bit of grepping.

It's AMAZINGLY not as horrible as I thought it would be: only 267 references
to errno in the whole library.

I guess I have to look into how to do git submodules right.
** Register usage in libc

As I suspected, 6 xmm registers are essentially unused and it would be
straightforward to kill two more. And maybe - if we can switch to
old style floating point elsewhere - use 'em all up. :) 

And we get back hardware sin, cos.

dave@nemesis:~/git/ebusl/lib$ for i in `seq 0 15`; do echo -n "xmm$i "
; objdump -d libc.a | grep xmm$i | wc -l; done

xmm0 3274 xmm1 2598 xmm2 1802 xmm3 1103 xmm4 726 xmm5 417 xmm6 281
xmm7 207 xmm8 30 xmm9 12 xmm10 6 xmm11 0 xmm12 0 xmm13 0 xmm14 0 xmm15
0

it's a bit less encouraging on the regular register front. I didn't go
looking for the short versions of these. They are used a lot, too.

%rax 8394 rbx 4902 rcx 2312 rdx 4202

%rdi rsp esi rbp

%r8 1764 r9 937 r10 598 r11 317 r12 2836 r13 2119 r14 1606 r15 1408

While I'm thinking about abusing floating point in general

-fno-trapping-math, -fno-signaling-nans

https://gcc.gnu.org/wiki/FloatingPointMath

And I just hopefully got rid of another errno headache:
...
gcc -mfpmath=387 -ffast-math -O3 -fverbose-asm -S ssevs87.c -lm

        movsd   %xmm0, -8(%rsp) # a, %sfp
        fldl    -8(%rsp)        # %sfp
        movsd   %xmm1, -8(%rsp) # b, %sfp
        fldl    -8(%rsp)        # %sfp
        fmul    %st(0), %st     #,
        faddp   %st, %st(1)     #,
        fsin
...
If I can just get rid of printf.

musl recompiled with those options (I'm scared to try it)
...
Calling it with omit-regs didn't do what I thought it would do. What I think I'm seeing is some inline
assembly or something that uses this up on the crypto front.
...
And with changing the calling convention I'm pretty sure this
kind of crazyness goes inline. (Not that what I'm talking about
isn't crazy in the first place). Hmm. musl also suports x32.

 2a3:   48 89 6c 24 10          mov    %rbp,0x10(%rsp)
 2a8:   f2 0f 11 44 24 08       movsd  %xmm0,0x8(%rsp)
 2ae:   f2 0f 10 44 24 10       movsd  0x10(%rsp),%xmm0
 2b4:   e8 00 00 00 00          callq  2b9 <csinh+0x2b9>

** heh. 80 bits is still better than 64... or so I thought
./blanusa_powint_64.dbg | grep inf | wc -l
226
./blanusa_powint_80.dbg | grep inf | wc -l
214

This made me worry about all the arithmetic out there today, but I'd 
got it wrong - using random numbers...

So I checked harder. :whew: For starters the compiler insisted on
vectorizing the math where it could (heh), until I recompiled with
-Os, which forced a call to the x87 function and the calling
convention sucks that way.

Oh what short opcodes there used to be for things like matrix
multiply!

 w  DB F1     F4X4            IIT only: 4 by 4 matrix multiply


Read more: http://www.intel-assembler.it/portale/5/The-8087-Instruction-Set/A-one-line-description-of-x87-instructions.asp#ixzz4cyO3yy59

* Hmm

I *was* obsessed by speed. Still am. On the other hand, I'm having
trouble expressing what I'm trying to express and perhaps switching
to lisp might help. There's three choices there - clojure is popular,
sbcl I knew best... and... emacs.

I have a lot of compelling reasons to want to launch back into emacs
for a while.  I feel as though my hands are cut off sometimes. I hate
switching to browsers where I used to have a keystroke for
something. There are bunch of little tools - example, I really want to
convert between an url and org and a md format thing with a keystroke,
I can never switch syntaxes.... gnugol could use some love... and so
on.

And the idea of doing packet processing in emacs just makes me
chortle.

* Fuschia

has a few things very like what I've been thinking about - capabilities,
and ha! has a threaded architecture that actually reads thread "regs"

https://fuchsia.googlesource.com/magenta/+/HEAD/docs/syscalls/thread_read_state.md
* Separate call and data stacks

The more I think about it, the idea of separate call and data stacks appeals
more - (virtually) eliminating ROP, as one objective. ERM doesn't have much
of a call or data stack as it is - args are passed in registers, messages
via ringbuffers, there's a lot of jumpy code (tail recursion). and so on.

But I wanted to stick with C as a base.
* Couple notes

Arm thus far is yielding to the kinds of thinsg I want to be doing
best, and bootstrapping the parallella looking hard. I'm also
discouraged on that front by their lead engineer going to work for
darpa which seems to mean to me that general availability of the 1000
core version may never happen. So I think I'll focus on arm for a while.

(I REALLY wanted DMA tho - 32 dma channels on the parallella - yum!)

I do really need the core infrastructure to land, or to at least be
spending time per day/week knocking out the basic boring bits, to just
reassemble and re-evolve them later. I'm close to having a kernel
route dumper after yesterday's hacking session... I'm getting mildly
better at writing code the compiler can optimize, also.

Instead, I keep thinking I need a new language (called "eh" in my
mind) to express things in, which is way more bootstrapping than I
want too! And to work on that state machine to verilog parser to
extend it to outputting C, also.
* TODO Tramp mode
  :LOGBOOK:
  CLOCK: [2017-04-14 Fri 07:20]--[2017-04-14 Fri 07:20] =>  0:00
  :END:
[2017-04-14 Fri 07:20]
* TODO INFIX notation is a plague upon the planet
and for that matter C++'s lack of being able to create new operators, also.

Since we are essentially creating new operators, and we have to lisp-like
notation throughout, life starts to get easier.

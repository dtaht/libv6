...
#define packet_err(e)                                                        \
  {                                                                          \
    debug_log_address(1, 1, e, addr);                                        \
    goto err;                                                                \
  }

//  extern fulladdr_t bogus_addr; ?

fulladdr_t classify_address(int ae,
                            int plen,
                            ...
                            unsigned char* p_r)
{
  ...
  */
  ...
  case 2:
    ...
    break;
  case 3:
    if(pb > 8 && len < pb - 8) packet_err("LL Packet omitted offset wrong");
    prefix[0] = 0xfe;
    prefix[1] = 0x80;
    addr.flags.ll = 1;
  ...
  //?     normalize_prefix(p_r, prefix, plen < 0 ? 128 : ae == 1 ? plen + 96 :
  // plen);
  addr.flags.len = plen;

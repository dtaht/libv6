...
static void sigexit(int signo) { exiting = 1; }

static void sigdump(int signo) { dumping = 1; }

static void sigreopen(int signo) { reopening = 1; }

static void sigcpu(int signo) { lowoncpu = 1; }
...
sigs_t mysignals[] = { { sigexit, SIGTERM },
                       { sigexit, SIGHUP },
                       { sigexit, SIGINT },
                       { SIG_IGN, SIGPIPE },
                       { sigdump, SIGUSR1 },

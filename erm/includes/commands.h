...
typedef enum {
  ...
  ROUTER_FILT_INTERFACES_ALL =
  (ROUTER_FILT_ME | ROUTER_FILT_DEAD | ROUTER_FILT_LAG | ROUTER_FILT_INTERFACES)
} router_filt_t;

// The second arg became perms. 0 is root required. 1 is membership in the erm
// group
// 2 is anyone that can connect. 3 everyone else. There may be more levels in
// the future.

...
// inline trap_t erm_get_trap_mask(trap_t); // boolean to only see the traps you want
...
// Note I am not using likely or unlikely on a few operations because of the extra
...
#define TRAP_LT(op,value,msg) if(unlikely((op) < (value)))  errcmd(msg);
#define TRAP_GT(op,value,msg) if(unlikely((op) > (value)))  errcmd(msg);
#define TRAP_EQ(op,value,msg) if(unlikely((op) == (value))) errcmd(msg);

// In c true is any non zero value. Remember that.

#define TRAP_TRUE(op,msg) if((op))              errcmd(msg)
#define TRAP_FALSE(op,msg) if(unlikely(!(op)))  errcmd(msg)

#define TRAP_WTRUE(op,msg) if((op))             wrncmd(msg)
#define TRAP_WFALSE(op,msg) if(unlikely(!(op))) wrncmd(msg)

#endif

#ifdef TRAP_BIG
#define TRAP_LT(op,value,msg) if(unlikely((op) < (value)))  errcmd(msg, __func__)
#define TRAP_GT(op,value,msg) if(unlikely((op) > (value)))  errcmd(msg, __func__)
#define TRAP_EQ(op,value,msg) if(unlikely((op) == (value))) errcmd(msg, __func__)
...
#define TRAP_TRUE(op,msg) if((op))                       errcmd(msg,__func__)
#define TRAP_FALSE(op,msg) if(unlikely(!(op)))           errcmd(msg,__func__)

#define TRAP_WTRUE(op,msg) if((op))                      wrncmd(msg,__func__)
#define TRAP_WFALSE(op,msg) if(unlikely(!(op)))          wrncmd(msg,__func__)
#define TRAP_WEQ(op,value,msg) if((op) == (value))       wrncmd(msg,__func__)

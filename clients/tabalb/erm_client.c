...
//        TRAP_WERR(shm_unlink(file), "Couldn't close shared memory"); // I can still delete myself here. Yuck. What can I do about it?
	memset(&file,0,sizeof(file));
        fd=0;
	mem = NULL;
	tsize = BASE*16;
        printf("erm instance detached\n"); // Hmm. I'm not seeing this on atexit aborted with cntrl-c
	return 0;
}
...
erm_t erm_attach_client(char *instance) {
	TRAP_LT((fd = shm_open(instance, O_RDONLY, 0)), 0, "Couldn't open shared memory - aborting");
	// struct stats_t; 
	// Hmm. Can we get the size from stat?
	TRAP_WEQ((mem = mmap(NULL,BASE*16,PROT_READ, default_perms, fd ,0)), (void *) -1, "Couldn't mmap shared huge page memory");
	if(mem == (void *) -1 ) {
		default_perms &= ~MAP_HUGETLB;
	        TRAP_EQ((mem = mmap(NULL,BASE*16,PROT_READ,default_perms , fd , 0)), (void *) -1, "Couldn't mmap shared memory - aborting");
	}
	atexit(bye);
...
uint32_t erm_status(erm_t erm) {
	return mem[8];
}

erm_t * erm_query(erm_t erm, uint32_t *buf, int size) {
	return &mem[9];
}

...
typedef struct { v6route, v6addr, v4route, v4addr, link } kernel_sockets_t;
...
/* Since we have a socket for v4 and v6, we are less complicated, but we still share code */

static int
parse_kernel_route_rta4(struct rtmsg *rtm, int len, struct kernel_route *route)
{
